...
Matrix3x3  obj_tools::operator * (const float& s, const Matrix3x3& m) {
    Matrix3x3 res;
...
Matrix3x3& Matrix3x3::operator =(float elements[3][3]) {
    for (int i = 0; i < MAX_ROWS; i++) {
        for (int j = 0; j < MAX_COLS; j++) {
            this->elements_[i][j] = elements[i][j];
        }
...
float Matrix3x3::getElement(const int& row, const int& col) {
...
Matrix3x3 Matrix3x3::transposition(void) {
    Matrix3x3 res;
